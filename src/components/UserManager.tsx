...
export default function UserManager({
  ...
  /** Hide / disable every mutating control. Server still enforces. */
  readOnly?: boolean;
}) {
  ...
  const [editRole, setEditRole] = useState<"user" | "viewer" | "admin">("user");
  ...
  function startEdit(u: U) {
    ...
    setEditRole(u.role as "user" | "viewer" | "admin");
    setEditPassword("");
    setEditErr(null);
  }
  ...
  async function quickRoleChange(id: number, role: "user" | "viewer" | "admin") {
    setMobileBusyId(id);
    setMobileErr(null);
    setMobileErrId(null);
    try {
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <select
          className="rounded-md border border-magic-border px-3 py-2 text-sm"
          value={role}
          onChange={(e) => setRole(e.target.value as "user" | "viewer" | "admin")}
        >
                ...
                <tr key={u.id} className="border-t border-magic-border bg-amber-50/50">
                  ...
                  <td className="p-2">
                    <select
                      ...
                      value={editRole}
                      onChange={(e) => setEditRole(e.target.value as "user" | "viewer" | "admin")}
                    >
                      <option value="user">user</option>
        ...
        {users.map((u) => (
          ...
          >
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {readOnly ? (
                <span className="rounded-md border border-magic-border px-2 py-1 text-sm bg-magic-soft text-magic-ink/70">
                  {u.role}
                </span>
              ) : (
                <select
                  ...
                  onChange={(e) =>
                    void quickRoleChange(u.id, e.target.value as "user" | "viewer" | "admin")
                  }
